...
namespace PruebaWeb.Data
{
    public class SDES
    {
        ...
        public void DecodificarSDES(HttpPostedFileBase postedFile, string ruta, string clave)
        {
            obtenerPermutaciones();
            int bufferLength = 8;
            using (var stream = new FileStream(postedFile.FileName, FileMode.Open))
            {
                using (var reader = new BinaryReader(stream))
                {
                    using (var writeStream = new FileStream(ruta + "\\Descodificado.txt", FileMode.OpenOrCreate))
                    {
                        using (var writer = new BinaryWriter(writeStream))
                        {
                            var byteBuffer = new byte[bufferLength];
                            while (reader.BaseStream.Position != reader.BaseStream.Length)
                            {
                                byte[] codificarFinal = new byte[bufferLength];
                                var contadorPosiconByte = 0;
                                //int[] codificarFinal = new int[bufferLength];
                                byteBuffer = reader.ReadBytes(bufferLength);
                                foreach (var a in byteBuffer)
                                {
                                    string k1 = generarK1(clave);
                                    ...
                                    codificarXor1 = XOR(codificarEP, k2, 8);
                                    ...
                                    var codificarSwapEPXor = XOR(codificarSwapEP, k1, 8);
                                    var S02 = string.Empty;
                                    var S12 = string.Empty;
                                    S02 = codificarSwapEPXor.Substring(0, 4);
                                    S12 = codificarSwapEPXor.Substring(4, 4);
                                    ...
                                    var codificarXorRestoSwap = XOR(codificarSBox2P4, codificarSwapPrimeraParte, 4);
                                    var union2 = codificarXorRestoSwap + codificarSwapSegundaParte;
                                    var codificarIpInversa = string.Empty;
                                    for (int i = 0; i < 8; i++)
                                    {
